...
//   Clock In / Clock Out → <input type="time">  (HH:MM 24h, native picker)
...
function updateEditValueInput() {
  ...
  const timeInput  = document.getElementById("edit-value-time");
...
window.openEditModal = function (date) {
  ...
  document.getElementById("edit-value-time").value    = "";
...
document.getElementById("submit-edit-btn").addEventListener("click", async () => {
  ...
  if (field === "Location") {
    ...
    val = document.getElementById("edit-value-time").value; // "HH:MM" 24h or ""
    if (!/^\d{2}:\d{2}$/.test(val)) return toast("Please pick a valid time", "error");
  }
